...
def run_list(graph, max_partition_size):
    ...
    while not quit:
        ...
        for i in range(nb_partitions):
            ...
            sorted(p, key = lambda pos: layout[pos].x()) if count % 2 == 0 else sorted(p, key = lambda pos: layout[pos].y())
...
def get_partitions(graph):
    return [p for p in graph.getSubGraphs() if p.getName().startswith("partition")]    
...
def main(graph):
    ...
    max_size = 0
    partitions = get_partitions(graph)
    for p in partitions:
        p_size = p.numberOfNodes()
