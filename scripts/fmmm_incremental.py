...
class FMMMIncremental:
    ...
    def run(self, root, multilevel=True, debug=False):
        ...
        if len(subgraphs) == 0:
            ...
            ds = tlp.getDefaultPluginParameters("Custom Layout", sg)
            sg.applyLayoutAlgorithm("Custom Layout", ds)
            ...
            ds = tlp.getDefaultPluginParameters("Custom Layout", subgraphs[0])
            subgraphs[0].applyLayoutAlgorithm("Custom Layout", ds)
            ...
            if multilevel:
                ...
                ds = tlp.getDefaultPluginParameters("Custom Layout", new_graph)
                ds["block nodes"] = True
                ds["movable nodes"] = new_graph.getBooleanProperty("canMove")
                new_graph.applyLayoutAlgorithm("Custom Layout", ds)
